...
func DefineMirrorCommand(kpApp *kingpin.Application) *kingpin.CmdClause {
	...
	runFunc := func() error {
		...
		finder := versions.NewVersionsComparer(source, destination, destListOptions, nil, nil, policyContext)
		...
		for _, src := range modulesImages {
			if err := copyImage(ctx, src, destination, policyContext, copyOpts...); err != nil {
				return err
			}
		}
